...
public class ALERT_Thread extends Application implements Runnable {
    ...
    public ALERT_Thread(NotificationChannel chan, NotificationManager man, NotificationCompat.Builder build,PendingIntent pending) {
    ...
    public void run(){
        ...
        while (!Thread.currentThread().isInterrupted() && BluetoothSocketShare.getBluetoothSocket().isConnected()) {
            ...
            try {
                ...
                LinkedList<Rpm> list= (LinkedList) adb.rpmDao.getAll();
                for (Rpm r:list) {
                    int speedValue = Integer.parseInt(r.getSpeed());
                    if (speedValue != 0) {
                        if (speedValue > 140 && !sendedSpeed) {

                            createNotification("BE CAREFUL!", "You're over the maximum speed limit!", R.drawable.tachimeter);
                ...
                if((covariance_soil>600000|| soil_counter>100) && !sendedRPM){
                    covariance_soil=0;
                    soil_counter=0;
                    createNotification("WARNING!","You have a reckless driving for a "+BluetoothSocketShare.getFuelType()+" car!",R.drawable.auto);
                    mNotificationManager.notify(0, mBuilder.build());
                    sendedRPM=true;
